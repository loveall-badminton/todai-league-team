<script lang="ts">
	...
	let xScale = $derived(
		series
			? d3.scaleLinear().domain([0, series.a.length - 1]).range([0, innerW])
			: d3.scaleLinear().range([0, innerW])
	);

	let yMax = $derived(series ? Math.max(5, d3.max([...series.a, ...series.b], (d) => d.y) ?? 5) : 5);
	...
	<div>
		<div class="flex items-center gap-3 mb-2">
			<span class="flex items-center gap-1 text-[10px] text-pink-700">
				<svg width="16" height="8"><line x1="0" y1="4" x2="16" y2="4" class="stroke-pink-600" stroke-width="2" /></svg>
				{nameA}
			</span>
			<span class="flex items-center gap-1 text-[10px] text-cyan-700">
				<svg width="16" height="8"><line x1="0" y1="4" x2="16" y2="4" class="stroke-cyan-600" stroke-width="2" /></svg>
				{nameB}
			</span>
		</div>
			...
			{#if width > 0 && height > 0}
				...
				>
					<g transform="translate({margin.left},{margin.top})">
						...
						<!-- X axis line -->
						<line x1={0} y1={innerH} x2={innerW} y2={innerH} class="stroke-zinc-200" stroke-width="1" />

						<!-- Score lines -->
						<path d={pathA} fill="none" class="stroke-pink-600" stroke-width="2" stroke-linejoin="round" />
						<path d={pathB} fill="none" class="stroke-cyan-600" stroke-width="2" stroke-linejoin="round" />

						<!-- Tooltip crosshair -->
						{#if tooltipIndex != null && tooltipX != null}
							<line
								x1={tooltipX} y1={0}
								x2={tooltipX} y2={innerH}
					...
					{#if tooltipIndex != null && tooltipCX != null && tooltipA != null && tooltipB != null}
						...
						<rect x={labelX} y={8} width={56} height={22} rx="4" class="fill-white stroke-zinc-200" stroke-width="1" />
						<text x={labelX + 6} y={23} font-size="11" font-weight="600">
							<tspan class="fill-pink-600">{tooltipA}</tspan><tspan class="fill-zinc-400" font-weight="400"> – </tspan><tspan class="fill-cyan-600">{tooltipB}</tspan>
						</text>
					{/if}
				</svg>
			{/if}
...
{:else}
	<p class="text-center text-sm text-zinc-400 py-6">まだラリーがありません</p>
{/if}
